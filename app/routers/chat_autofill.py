...
_AUTOFILL_SYSTEM_PROMPT_BASE = (
    ...
    "- The RFQ form is always in English. No matter what language the source text is written "
    "in (French, German, etc.), TRANSLATE every value into English before putting it into "
    "`updateFormFields` — free-text fields, product names, part descriptions, notes, "
    "everything. Never copy non-English text into the form as-is.\n"
    ...
    "- Whenever the text gives ANY quantity/volume information for a product — even just a "
    "single flat annual quantity (e.g. 'annual quantity of 500,000 pieces'), not an explicit "
    "multi-year table — you MUST ALSO populate the top-level `volumes` array with one entry "
    "for it, in addition to the product row: use the SOP year (or whatever year the quantity "
    "applies to) as the key in the `volumes` year→quantity map, and fill `target_price`, "
    "`price_source`, `delivery_zone`, `plant`, and `country` on that same volumes entry from "
    "whatever the text already gave for pricing/delivery. Never leave `volumes` empty just "
    "because the text presents the quantity as a single flat number instead of a table.\n"
    ...
    "`customer_tooling_conditions`, `entry_barriers`, `type_of_packaging`, `scope`, "
    "`strategic_note`, and `final_recommendation`, but applies to any other field with the "
    "same list-like shape), put EACH distinct item/sentence on its OWN line, separated by "
    "'\\n' newlines — never join multiple items into one run-on sentence with commas or "
    "semicolons. Mirror how the source text separates them (bullets, line breaks, or "
    "separate sentences each become one line).\n"
    ...
    "- `capacity_available` ('Technical Capacity') and `pays_for_development` ('Development "
    "Costs') normally expect a short value ('Yes'/'No' for the former, a responsible party "
    "for the latter), but the form lets the user type any free text as a custom answer — so "
    "never leave these blank just because the text doesn't match that expected shape. Fill "
    "them with whatever relevant description the text gives (e.g. 'capacity considered "
    "sufficient without major investment', 'estimated at a medium level') as free text; only "
    "omit them if the text says nothing at all about that topic."
)
